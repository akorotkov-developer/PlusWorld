...
if($_REQUEST["CONFIRM_CODE"] <> "" && $ID > 0 && empty($_REQUEST["action"]))
{
	if($arSubscription["CONFIRMED"] <> "Y" && count($arWarning)==0)
	{
        ...
        while(($subs = $subscription->Fetch()))
            $email = $subs["EMAIL"];

        $data = array("EMAIL"=>$email, "SUBSCRIBE_LIST"=>array(1));
        \Bitrix\Sender\Subscription::subscribe($data);
        ...
        $sql = "SELECT ID FROM b_sender_contact WHERE EMAIL = '".$email."'";
        ...
        $Empty = false;
        $sql = "SELECT LIST_ID FROM b_sender_contact_list WHERE 'CONTACT_ID' = '".$ID."'";
        $isEmpty = $connection->query($sql);
        while(($res = $isEmpty->Fetch()))
            $Empty = $res;
        if (!$Empty) {
            $sql = "INSERT INTO b_sender_contact_list (CONTACT_ID, LIST_ID) VALUES ('" . $ID . "', '1')";
	...
	case "unsubscribe":
		if(CSubscription::IsAuthorized($ID))
		{
			...
			if($obSubscription->Update($ID, array("ACTIVE"=>"N")))
			{
                ...
                $sql = "DELETE FROM b_sender_contact WHERE ID = '".$ID."'";
                $connection->query($sql);
                $sql = "DELETE FROM b_sender_contact_list WHERE CONTACT_ID = '".$ID."'";
                $connection->query($sql);
			}
		}
		break;
