...
if($arParams["SHOW_WORKFLOW"] || $this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()),$bUSER_HAVE_ACCESS, $arNavigation)))
{
	...
	if($obElement = $rsElement->GetNextElement())
	{
        ...
        $obJournal = CIBlockElement::GetList(array(), array(
                "IBLOCK_ID" => $arResult["PROPERTIES"]["JOURNAL"]["LINK_IBLOCK_ID"],
                "ID" => $arResult["PROPERTIES"]["JOURNAL"]["VALUE"],
            ), false, false, array("ID", "NAME", "IBLOCK_ID","DETAIL_PAGE_URL"));
            if ($arJournal = $obJournal->GetNext())
            {
                  $APPLICATION->AddChainItem($arJournal["NAME"], $arJournal["DETAIL_PAGE_URL"]);
            }
