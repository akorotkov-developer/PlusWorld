...
$arParams["NUM_PAGE"] = (intval($arParams["NUM_PAGE"]) > 0 ? intval($arParams["NUM_PAGE"]) : 1);
$arParams["NUM_NEWS"] = (intval($arParams["NUM_NEWS"]) > 0 ? intval($arParams["NUM_NEWS"]) : 20);

$arParams["PDF_FIELD"] = trim($arParams["PDF_FIELD"]);
...
if($this->StartResultCache(false, array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups(), $arrFilter)))
{
	...
	if($arParams["NUM_NEWS"]>0)
		$limit["nPageSize"] = $arParams["NUM_NEWS"];

    if($arParams["NUM_PAGE"]>0)
		$limit["iNumPage"] = $arParams["NUM_PAGE"];

	$arResult["ITEMS"]=array();

    if ($limit["nPageSize"] > 0 && $limit["iNumPage"] > 0)
    {
        $limit_from = $limit["nPageSize"] * $limit["iNumPage"] - $limit["nPageSize"];


    	$rcntElements = CIBlockElement::GetList($arSort, array_merge($arFilter, $arrFilter), array(), false, $arSelect);

        if ($limit_from <= $rcntElements)
    	{
        	CTimeZone::Disable();
        	...
        	while($obElement = $rsElements->GetNextElement())
        	{
        		...
        		if(strlen($arResult["NODES"]["description"])>0)
        			...
        			$arItem["description"]=htmlspecialchars(($arElement["PREVIEW_TEXT"] || $arParams["YANDEX"]) ? $arElement["PREVIEW_TEXT"] : '');
                ...
                if (!empty($arParams["PROPERTY_FIELDS"]))
                {
                    foreach ($arParams["PROPERTY_FIELDS"] AS $prop)
                    {
                        $arItem["PROPERTY"][$prop] = $arProperties[$prop]["VALUE"];
