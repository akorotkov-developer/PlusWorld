	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			if($arParams["PREVIEW_TRUNCATE_LEN"] > 0)
				$arItem["PREVIEW_TEXT"] = truncate(strip_tags($arItem["PREVIEW_TEXT"]), $arParams["PREVIEW_TRUNCATE_LEN"], "...");
            ...
            if ($arItem["IBLOCK_SECTION_ID"] > 0)
            {
                ...
                $arItem["SECTION"] = $section->GetNext();
            }
...
if(isset($arResult["ID"]))
{
	...
	if($arParams["SET_TITLE"])
	{
	   if ($arResult["SECTION"]["PATH"][0]["NAME"])
        $APPLICATION->SetTitle($arResult["SECTION"]["PATH"][0]["NAME"], $arTitleOptions);
        else $arResult["NAME"];
