...
<?$frame = $this->createFrame()->begin("");//Начало динамической области?>
    <script type="text/javascript" src="/bitrix/templates/plus/js/jquery.form.min.js"></script>
...
<table class="profile-table data-table">
	...
	?>
	<tr>
		<td class="pad_5"><?=GetMessage('NAME')?><span class="starrequired">*</span></td>
		<td><input type="text" required="" name="NAME" maxlength="50" value="<?=$arResult["arUser"]["NAME"]?>" /></td>
	</tr>
	<tr>
		<td class="pad_5"><?=GetMessage('LAST_NAME')?><span class="starrequired">*</span></td>
		<td><input type="text" required="" name="LAST_NAME" maxlength="50" value="<?=$arResult["arUser"]["LAST_NAME"]?>" /></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><input type="email" required="" name="EMAIL" maxlength="50" value="<? echo $arResult["arUser"]["EMAIL"]?>" /></td>
	</tr>
	<tr>
		<td><?=GetMessage('LOGIN')?><span class="starrequired">*</span></td>
		<td><input type="text" required="" name="LOGIN" maxlength="50" value="<? echo $arResult["arUser"]["LOGIN"]?>" /></td>
	</tr>
	<tr>
...
<?endif?>
	...
	<tbody>
		<tr>
			<td><?=GetMessage('USER_PHONE')?><span class="starrequired">*</span></td>
			<td><input type="text" required="" name="PERSONAL_PHONE" maxlength="255" value="<?=$arResult["arUser"]["PERSONAL_PHONE"]?>" /></td>
		</tr>
        <tr>
            <td><?=GetMessage('USER_COMPANY')?><span class="starrequired">*</span></td>
            <td><input type="text" required="" name="WORK_COMPANY" maxlength="255" value="<?=$arResult["arUser"]["WORK_COMPANY"]?>" /></td>
        </tr>
        <?
        global $USER;
        $idUF_TYPE_ORGANIZATION = 0;
        $rsData = CUserTypeEntity::GetList(array(), array("FIELD_NAME" => "UF_TYPE_ORGANIZATION") );
        while($arRes = $rsData->Fetch())
        {
            $idUF_TYPE_ORGANIZATION = $arRes["ID"];
        }
        if ($_REQUEST["rt"]=="rt") {
            if ($idUF_TYPE_ORGANIZATION>0) {?>
            <tr>
                <td>Тип Вашей организации:<span class="starrequired">*</span></td>
                <td>
                    <?
                    $arEnumPlusRL = array();
                    $rsEnum = CUserFieldEnum::GetList(array("SORT"=>"ASC"), array("USER_FIELD_ID" => $idUF_TYPE_ORGANIZATION) );
                    while($arEnum = $rsEnum->GetNext())
                    {
                        array_push($arEnumPlusRL,$arEnum);
                    }
                    ?>
                    <select required="" class="typeselect" name="UF_TYPE_ORGANIZATION" id="UF_TYPE_ORGANIZATION">
                        <?foreach ($arEnumPlusRL as $arEnum) {?>
                            <option value="<?=$arEnum["ID"]?>" <?if ($arResult["arUser"]["UF_TYPE_ORGANIZATION"]==$arEnum["ID"]) echo "selected"?>><?=$arEnum["VALUE"]?></option>
                        <?}?>
                    </select>
                </td>
            </tr>
		...
		<tr>
			<td><?=GetMessage('USER_WWW')?><span class="starrequired">*</span></td>
			<td><input type="text" required="" name="WORK_WWW" maxlength="255" value="<?=$arResult["arUser"]["WORK_WWW"]?>" /></td>
		</tr>
		<tr>
		...
		<tr>
			<td><?=GetMessage('USER_POSITION')?><span class="starrequired">*</span></td>
			<td><input type="text" required="" name="WORK_POSITION" maxlength="255" value="<?=$arResult["arUser"]["WORK_POSITION"]?>" /></td>
		</tr>
