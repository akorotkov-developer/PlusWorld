...
<?endif?>
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" class="form" method="post" enctype="multipart/form-data">
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
								<input type="text" id="label<?=$propertyID?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" style="width:100%;" value="<?=$value?>" /><br /><?
	...
	<input type="hidden" name="PROPERTY[IBLOCK_SECTION][]" value="929" />
	<input type="hidden" name="PROPERTY[CODE][]" value="add<?=time();?>" />
</form>
