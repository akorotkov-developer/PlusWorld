...
$resArticles = CIBlockElement::GetList(array("DATE_ACTIVE_FROM"=>"DESC"), array("IBLOCK_ID"=>$arParams["ARTICLES_IBLOCK_ID"],"PROPERTY_KNOW_BASE"=>$sectionID), false, false, array("ID","IBLOCK_ID","NAME","PREVIEW_PICTURE"));
$i = 0;
...
<?if($arItem["ARTICLES"]):?>
	...
	<?foreach($arItem["ARTICLES"] as $k => $arArticle):?>
		...
		<div class="kb_attach_article_item">
			<div class="kb_attach_article"><a href="/journal/online/art<?=$arArticle['ID']?>/"><?if($file['src']):?><img class="kb_article_picture" src="<?=$file['src']?>" /><?endif;?><?=$arArticle['NAME']?></a></div>
		<div class="clear"></div>
		</div>
	<?endforeach;?>
	<hr />
