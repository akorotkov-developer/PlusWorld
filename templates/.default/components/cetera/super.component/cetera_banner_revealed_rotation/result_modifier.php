...
if(!isset($arParams['TRANSITION_INTERVAL'])) {
    $arParams['TRANSITION_INTERVAL'] = 15000;
}
...
$arBANNERS = array();
$Type = $arParams['TYPE_SMALL'];
$TypeBig = $arParams['TYPE_BIG'];
if ($TARGETING) {
    ...
    while($ar_fields = $resBANNERS->GetNext()) {
        if (strlen($Type)>0) {
            if ($ar_fields["TYPE_SID"]==$Type) {
                ...
                if ($arBannerID["LAMP"] == "green") {
                    $ar_fields['FIELDS'] = $arBannerID;
                    $ar_fields['HTML'] = CAdvBanner::GetHTML($arBannerID);
                    if (intval($arBannerID["COMMENTS"]) > 0) {
                        $rsBannerIDBig = CAdvBanner::GetByID(intval($arBannerID["COMMENTS"]), false);
                        $arBannerIDBig = $rsBannerIDBig->Fetch();
                        if (($arBannerIDBig["TYPE_SID"] == $TypeBig)and($arBannerIDBig["LAMP"] == "green")) {
                            $arBannerIDBig["HTML"] = CAdvBanner::GetHTML($arBannerIDBig);
                            $ar_fields["BIG"] = $arBannerIDBig;

                            $arFixBannerTemp = $arFixBanner;
                            $arFixBannerTemp["ID"] = intval($arBannerID["COMMENTS"]);
                            CAdvBanner::FixShow($arFixBannerTemp);
                        }
                    }
                ...
                $arFixBannerTemp = $arFixBanner;
                $arFixBannerTemp["ID"] = intval($ar_fields["BANNER_ID"]);
                CAdvBanner::FixShow($arFixBannerTemp);
            }
        }
        else
