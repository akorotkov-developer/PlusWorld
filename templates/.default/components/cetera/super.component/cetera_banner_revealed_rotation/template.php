    ...
    <div id="<?=$block_id;?>" class="b-banners__bottom-lenta <?=$arResult['SLIDER_CSS_CLASS']?>" style="<?if($arParams['WIDTH']){?>width:<?=$arParams['WIDTH'];?>;<?}?><?if($arParams['HEIGHT']){?>height:<?=$arParams['HEIGHT'];?>;<?}?>">
        ...
        <div class="cetera-banner_slider-wrapper">
            <?foreach($arResult['BANNERS'] as $arBanner){?>
                ...
                <?}else{?>
                    <div class="stop-cetera-banner_slider-item cetera-banner_slider-item cetera-banner_slider-item_small-<?=$arBanner['FIELDS']['ID']?>" <?if (isset($arBanner["BIG"])) {?>data-big_id="<?=$arBanner["BIG"]['ID']?>"<?}?> data-banner_id="<?=$arBanner['FIELDS']['ID']?>"><?=$arBanner['HTML'];?></div>
                    <?if (isset($arBanner["BIG"])) {?>
                        <div class="stop-cetera-banner_slider-item cetera-banner_slider-item_big cetera-banner_slider-item_big-<?=$arBanner["BIG"]['ID']?>" data-small_id="<?=$arBanner['FIELDS']['ID']?>" data-banner_id="<?=$arBanner["BIG"]['ID']?>"><?=$arBanner["BIG"]['HTML'];?></div>
                    <?}?>
                <?}?>
            <?}?>
        </div>
...
<?if(is_array($arResult['BANNERS']) && !empty($arResult['BANNERS'])){?>
    <script type="text/javascript">
        (function($) {
           ...
           if ($(".cetera-banner_slider-item_big").length >= 1) {
                $(".cetera-banner_slider-item").mouseover(function () {
                    if ($(this).data("big_id") > 0) {
                        $(this).css("visibility", "hidden");
                        $(".cetera-banner_slider-item_big-" + $(this).data("big_id")).show();
                        $('.cetera-banner_slider').css("overflow", "visible");
                    }
                });

                $(".cetera-banner_slider-item_big").mouseout(function () {
                    $(this).hide();
                    $('.cetera-banner_slider').css("overflow", "hidden");
                    $(".cetera-banner_slider-item_small-" + $(this).data("small_id")).css("visibility", "visible");
                });
            };

            var obCeteraRotation = new Cetera_Banner_Rotation ({
                id : '<?=$block_id;?>',
            ...
            obCeteraRotation.onAfterShowBanner = function(banner_index) {
