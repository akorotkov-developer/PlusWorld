...
<?if (isset($_REQUEST["where"]) && $_REQUEST["where"] != 'news') {} else { ?>
    <dt <?=(isset($_REQUEST["where"]) && $_REQUEST["where"] == 'news' ? 'class="selected" style="font-weight: bold"': 'style="font-weight: bold"')?>><?=GetMessage("CT_NEWS_TAB")?> (<?if ($arResult["NAV_RESULT"]) echo $arResult["NAV_RESULT"]->SelectedRowsCount(); else echo 0?>)</dt>
