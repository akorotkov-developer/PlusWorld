...
<?if (isset($_REQUEST["where"]) && $_REQUEST["where"] != 'articles') {} else { ?>
    <dt <?=(isset($_REQUEST["where"]) && $_REQUEST["where"] == 'articles' ? 'class="selected" style="font-weight: bold"': 'style="font-weight: bold"')?>><?=GetMessage("CT_ARTICLES_TAB")?> (<?if ($arResult["NAV_RESULT"]) echo $arResult["NAV_RESULT"]->SelectedRowsCount(); else echo 0?>)</dt>
